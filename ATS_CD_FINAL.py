...
def get_gemini_response(job_description, resume_text, jd_match):
    # Determine Position Match based on JD Match percentage
    position_match = "Yes" if jd_match >= 40 else "No"  # Set the threshold as per requirement
...
if submit:
    if uploaded_resume is not None and uploaded_job_description is not None:
        ...
        response = get_gemini_response(job_description_text, resume_text, jd_match)
